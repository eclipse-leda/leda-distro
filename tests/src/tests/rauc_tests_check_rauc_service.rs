...
pub struct RaucTestCheckRaucService {
    pub test_name: &'static str,
}


#[async_trait]
impl TestParser for RaucTestCheckRaucService {
    async fn run_test(&self) -> TestCase {
        let start = SystemTime::now();

        let result = test_utils::is_active_system_service("rauc");
